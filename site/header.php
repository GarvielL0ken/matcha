<?php
	...
	session_start();
	if ($page != 'Login' && $page != 'Registration' && $page != 'Reset Password' && $page != 'Verify Email')
	{
...
<html>
	<head>
		...
		<body>
			...
			<?php
				$p = array('Chat' => 0, 'Login' => 0, 'Notifications' => 0, 'Profile' => 0, 'Registration' => 0, 
							'Reset Password' => 0, 'Search' => 0, 'Suggestions' => 0, 'View' => 0, 'Verify Email' => 0);
				$p[$page] = 1;
				if (!$p['Login'] && !$p['Registration'] && !$p['Reset Password'] && !$p['Verify Email'])
				{
				...
				if ($p['Login'] || $p['Reset Password'] || $p['Verify Email'])
					print(output_a('registration.php', 'Register'));
				if ($p['Registration'] || $p['Reset Password'] || $p['Verify Email'])
					print (output_a('login.php', 'Login'));
				if ($p['Login'] || $p['Profile'] || $p['Verify Email'])
